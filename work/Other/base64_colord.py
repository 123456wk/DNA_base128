...
def merge_data(final_data1):
    final_data_last = []
    final_data = ""
    sum1 = len(final_data1[-2]) * 3
    #   step1 ： 将获得的数组重新分组
    for i in range(math.ceil(len(final_data1) / 3)):
        final_data_five = ""
        remainder = len(final_data1) % 3
        devider = math.floor(len(final_data1) / 3)
        if i <= devider - 1:
            for j in range(3):
                final_data_five = final_data_five + "".join(final_data1[i * 3 + j])

        if i == math.ceil(len(final_data1) / 3) - 1 and remainder > 0:
...
def Reconstruct_iamge(image , error_rate):
