...
if __name__ == '__main__':
    ...
    #  随机错误加入
    final_data1 = random_error_add.error_add(Dna_data_str, 0.0025)
    ...
    for i in range(math.ceil(len(final_data1) / 5)):
        ...
        if len(final_data_five) != 750:
            print(final_data_five, len(final_data_five), "*******")

        if len(final_data_five) > 750:
            final_data_five = final_data_five[: 750]
        if len(final_data_five) < 750:
            length = 750 - len(final_data_five)
            for t in range(length):
                final_data_five = final_data_five + "A"
        # print(final_data_five, "final_data_five")
        final_data = final_data + final_data_five
